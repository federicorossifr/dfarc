...
def reverse_map(rp,Lx,Ly,op,rely=False,bits=4):
    rev = dict()
    pp_ind = 0
    last_ind = 0
    not_found = 0
    ...
    for i,p in enumerate(rp):
        for p1 in rp[i+1:]:
            ...
            try:
                ...
                pp_ind = rp.index(op(p1,p))+1

            except:
                not_found = not_found+1
                if rely and op(p1,p) in r8:
                    pp_ind = round((r8.index(op(p1,p))+1)/(2**(8-bits)))
                else:
                    pp_ind = last_ind
...
def main():
    ...
    if args.bits == "4":
        ...
        r = r8
    rev = reverse_map(r,Lx,Ly,operator.mul,True,int(args.bits))
    print(args.prefix)
